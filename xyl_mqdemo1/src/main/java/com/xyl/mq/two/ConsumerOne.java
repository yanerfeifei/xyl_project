...
public class ConsumerOne {
    public static void main(String[] args) throws Exception {
        ...
        //在持久订阅的时候需要一个id来标识
        connection.setClientID("xyl_mq02");
        //开启连接
        connection.start();
        //创建一个session
        ...
        //创建一个消费者
        MessageConsumer consumer = session.createDurableSubscriber(topic,"xyl_topic_02");
        //接收消息
        TextMessage receive = (TextMessage) consumer.receive();
        System.out.println("consumer2接收到的消息是："+receive.getText());
        //关闭资源
        consumer.close();
        session.close();
        connection.close();
